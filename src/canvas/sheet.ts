...
export function getAffectedUsers(assignment: string): AutoRefunds {
  ...
  const submissions : Map<string, string>  = getSubmissions(roster.map((r) => r[rHeaders['Canvas_id']]), assignment)
  const usedHeaderCol = sheet.usedHeader(assignment), freeHeaderCol = sheet.freeHeader(assignment)
  users.forEach((row) => {
    let useRequested = Number(row[headers[usedHeaderCol]]), free = Number(row[headers[freeHeaderCol]])
    let userId = row[headers[sheet.idHeader]]
    let userRosterDetails = roster.find(r => r[rHeaders[sheet.idHeader]]==userId)
    if (useRequested>0 && userRosterDetails) {
      let submittedTime = submissions.get(userRosterDetails[rHeaders['Canvas_id']])
      if (submittedTime) {
        let usedActual = timeDiff(submittedTime, config.assignments[assignment as Assignment].deadline)
