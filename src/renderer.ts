...
export function getEmailSubject(reqAct: string, res: Response) : string {
  if (reqAct === "summary")
    return literal.summary.subject();
  return literal.default.subject(reqAct, res.assignment, res.success, res.state.new?.deadline);
}

export function renderHTML(req: form.Request, res: Response): string {
  var template = Handlebars.compile(baseTemplate);

  const spendLiteral=(res.updateSummary.usage.length>0) ?
  ...
  summary.usageLiteral= (res.updateSummary.usage.map(u=>{
    ...
    container.comments = `${u.used + u.free} day(s)` + (u.free > 0 ? ` (including ${u.free} free late day(s))` : ``);
