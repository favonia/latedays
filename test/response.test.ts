...
jest.mock("../config/config", () => {
  const moduleMock = jest.requireActual("../config/config");
  let testConfig = {
    ...moduleMock.config,
    assignments: {
...
// Mocking variables and constants ends. Test cases begin from here.
...
describe("testing Responses", () => {
  ...
  test("responds appropriately when requesting refund beyond policy refund days", () => {
    ...
    expect(updateAndRespond(testEntry, refundRequest).comments).toStrictEqual(
  ...
  test("responds appropriately when requesting refund for an assignment with no used late days", () => {
    ...
    expect(updateAndRespond(refundEntry, refundRequest).comments).toStrictEqual(
      literalWrapper(literal.refUnusedBody)({
        assignment: defaultAssignment,
        oldDeadline: DateTime.fromISO(
          config.assignments[defaultAssignment].deadline
        ),
      })
    );
  });
  ...
  test("responds & refund request recieved before the deadline", () => {
    ...
    expect(refundEntry.days[defaultAssignment].used).toBe(
      usedDays - refundDays
    ); // 1 day refunded successfully
  });
  ...
  test("responds appropriately when requesting late days after the latest possible day", () => {
    ...
    expect(updateAndRespond(testEntry, reqRequest).comments).toStrictEqual(
  ...
  test("responds appropriately when requesting late days when no days are left", () => {
    ...
    expect(updateAndRespond(refundEntry, reqRequest).comments).toStrictEqual(
